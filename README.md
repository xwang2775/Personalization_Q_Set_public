...
An online study platform investigating the minimal set of questions effective
for an LLM to produce better-adapted responses to the user.
...
**Method:**
1. Participants complete a 80-item profiling questionnaire spanning:
   - Communication dimension vignettes (tone, initiative, verbosity, structure)
   - Proxy questions
   - Personality scales (Primal World Beliefs, Big 5)
...
**Analysis:** LASSO regression and Shapley values on the binary item-inclusion indicators → identify which profile items predict
satisfaction/usability/usefulness etc. improvement.
...
npm install
npm start
# REACT_APP_API_URL=https://your-backend-url.com npm run deploy
# REACT_APP_API_URL=https://muscly-tracheidal-dorie.ngrok-free.dev npm run deploy
```
